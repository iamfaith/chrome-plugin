...
document.addEventListener("input", function (event) {
    ...
    // Get the stored data from chrome storage
    chrome.storage.local.get("data", function (result) {
        ...
        for (var i = 0; i < filteredData.length; i++) {
            ...
            // var suggestions = input.parentNode.childNodes.item('suggestions')
            // if (!suggestions) {
...
// // Listen for blur events on the input element
// document.addEventListener("blur", function () {
//     // Get the input value
//     var value = input.value;

//     // Save the input value to chrome storage if it's not empty
//     if (value !== "") {
//         chrome.storage.local.get("data", function (result) {
//             // Get the data array or create a new one if it doesn't exist
//             var data = result.data || [];

//             // Add the input value to the data array if it's not already there
//             if (!data.includes(value)) {
//                 data.push(value);
//             }

//             // Save the data array to chrome storage
